...
Global, one line per run, across every domain. Append the newest entry at the top. Detail for any
single run belongs in that artifact's own `## Timeline` (see `knowledge/README.md`) — this file is
just "what ran, when, and where to look," kept short enough to skim.

Format: `YYYY-MM-DD HH:MM | domain | one-line summary | link(s)`
